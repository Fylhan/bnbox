...
class Router
{
    ...
    public function __construct(Container $container, $controller = '', $action = '')
    {
        ...
        $this->controller = $this->sanitize(empty($_GET['controller']) ? $controller : $_GET['controller'], 'bnog');
        $this->action = $this->sanitize(empty($_GET['action']) ? $action : $_GET['action'], 'index');
    }
     ...
     * @return string
     */
    private function sanitize($value, $default_value)
    {
